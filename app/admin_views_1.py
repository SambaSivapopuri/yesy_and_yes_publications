...
def nav_items(request):
    ...
    return render(request,"admin_templates/menu_items.html",{"data":Nav_bar.objects.all()})
@login_required
...
def add_sub_category(request):
    if request.method == "POST":
        ...
        if image:  # Ensure an image is uploaded
            try:
                Sub_Category.objects.create(name=name, status=status, image=image,category_id=category)
            except:
                return render(request, 'admin_templates/add_sub_category.html',{"error":"Sub-Category Exits.."})
        return redirect("admin")
...
def download_orders(request, format):
    ...
    if format == 'csv':
        ...
        for product in product_list:
            writer.writerow([product.id, product.customer, product.amount, product.status])
        return response

    elif format == 'excel':
        df = pd.DataFrame.from_records(product_list.values('id', 'customer__name', 'amount', 'status'))
        ...
        p.drawString(100, 750, "Order List")
        y = 730

        for product in product_list:
            p.drawString(100, y, f"ID: {product.id}, Customer: {product.customer}, Amount: {product.amount}, Status: {product.status}")
            y -= 20
